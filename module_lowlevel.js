const ModuleLowLevel = {
    description: "The heart of C — and the reason it's different from every language you've used before. Pointers are the mechanism that gives C its power and its danger: direct, unmediated access to memory addresses. Dynamic memory allocation with malloc/calloc/realloc/free is how C handles data structures whose size isn't known at compile time. Function pointers are how C does polymorphism and callbacks. Memory alignment explains why padding exists and why cache efficiency matters. And compiler diagnostics — warnings, sanitizers, and GDB — are the professional tools you need to find the bugs that C's low-level nature makes possible. This module is where C stops being a language and starts being a discipline.",
    
    lessons: [
        {
            id: "pointers-intro",
            title: "Introduction to Pointers",
            explanation: "Pointers are the feature that makes C fundamentally different from every other language you have likely encountered. A pointer is a variable that stores a memory address — not a value, but the location where a value lives. Every variable in RAM sits at a specific numbered address. A pointer stores that number. This is not an abstraction or a metaphor: you can print the address as a hexadecimal number, do arithmetic on it, pass it to a function that then modifies the original data through it. Languages like Python and Java have references, but those references are managed, restricted, and invisible — the runtime decides where objects live and moves them around as needed. C pointers are raw, unmediated addresses: the exact same thing the CPU uses internally. Understanding pointers is not optional in C. A program that doesn't use pointers is a calculator, not software. Dynamic memory allocation uses pointers. Passing large structs without copying uses pointers. Linked lists, trees, hash tables, graphs — all require pointers. Functions that return multiple values use output pointers. Every OS API you'll ever call uses pointers. Pointers are C.",
            sections: [
                {
                    title: "The Two Key Operators",
                    content: "Two operators control everything about pointers. You will type them thousands of times. They reuse symbols you already know from other contexts, but in the context of pointers they have completely different meanings — and the distinction matters enormously.",
                    points: [
                        "<strong>& (Address-of Operator)</strong>: Applied to a variable, produces that variable's memory address. <code>&num</code> doesn't give you the value stored in <code>num</code> — it gives you the memory address where <code>num</code> lives. Think of your variable as a house and its address as the street address. <code>&num</code> is the street address. You can hand it to someone (a function) and they can go to that address and read or modify what's inside.",
                        "<strong>* (Dereference Operator)</strong>: Applied to a pointer, follows the stored address and accesses the value at that location. <code>*ptr</code> says 'go to the address stored in ptr and give me what's there'. Using the house analogy: if <code>ptr</code> holds the street address, <code>*ptr</code> is going to that house and looking inside. The same <code>*</code> symbol also appears in pointer declarations — context tells you which meaning applies."
                    ]
                },
                {
                    title: "Creating a Pointer",
                    content: "Declaring a pointer looks like a normal variable declaration but with a <code>*</code> between the type and the name. The type matters critically — an <code>int*</code> is a pointer to an int, a <code>char*</code> is a pointer to a char. The type tells C two things: how to interpret the bytes at the pointed-to address, and how many bytes to jump when doing pointer arithmetic (<code>ptr + 1</code> on an <code>int*</code> moves 4 bytes forward, not 1). <strong>Always initialize a pointer when you declare it</strong>. An uninitialized pointer holds whatever bytes were in that memory slot before — a random, unpredictable address. Dereferencing it reads or writes some random location in memory, which can corrupt data or crash the program with no clear error message. If you don't have a valid address to assign yet, assign <code>NULL</code>.",
...
int main() {
    ...
    return 0;
        ...
        },
        {
            ...
            explanation: "Once compiled, a function is just machine code sitting at a specific address in memory. Like any other address, that address can be stored in a variable — a function pointer. You can pass that pointer to another function, store it in an array, or call through it at runtime. This unlocks one of the most powerful patterns in systems programming: passing behaviour as data. The C standard library's <code>qsort</code> can sort an array of any type — integers, strings, structs, anything — because you pass it a comparison function as a pointer. <code>qsort</code> doesn't know or care what it's sorting; it just calls your comparison function to decide order. The same mechanism powers Linux's virtual file system layer (every file system registers function pointers for open, read, write, close), GUI event systems (register a function to be called when a button is clicked), plugin architectures, state machines, and every callback-based OS API you'll ever call. Function pointers are how C achieves runtime polymorphism without objects.",
            sections: [
                {
                    title: "The Syntax",
...
sequence at offset:  4`,
        ...
        },
        {
            ...
            explanation: "The compiler knows far more about your code than you might expect. It can see signed-unsigned mismatches, variables read before initialization, functions called with the wrong number of arguments, unreachable code, implicit narrowing conversions that silently discard data, and dozens of other patterns that are almost always bugs. The problem is that the compiler stays quiet about them by default — in its standard configuration it prioritizes getting a binary out the door over being maximally helpful. The flags <code>-Wall -Wextra -Wpedantic</code> turn on the full diagnostic suite. <code>-Werror</code> promotes every warning to a hard compilation error, forcing you to address them. Professional C codebases compile with all these flags and treat zero warnings as a non-negotiable baseline. A codebase that compiles cleanly with <code>-Wall -Wextra -Werror</code> has provably eliminated an entire class of bugs before a single test has been written. Pair this with runtime sanitizers — AddressSanitizer and UBSan — and you have a two-layer defense that catches nearly every memory safety bug during development, before it ever ships.",
            sections: [
                {
...
READ of size 4 at 0x... thread T0
  #0 main prog.c:15`,
        ...
        },
        {
            ...
            explanation: "When a program crashes or produces wrong output, beginners instinctively reach for <code>printf</code> — inserting print statements to triangulate where things go wrong. This works for isolated, simple cases and is completely impractical for anything else. A segfault inside a 20-level call chain, a variable corrupted somewhere across 10,000 lines of code, a bug that only reproduces with specific input — these situations require a debugger. GDB (GNU Debugger) lets you stop a running program at any exact line, inspect every variable and register, step through execution one instruction at a time, examine the full call stack showing exactly how the program got to where it is, set watchpoints that trigger automatically when a specific variable changes, and catch a crash at the precise moment it occurs with the complete program state intact. Learning a dozen GDB commands replaces hours of futile printf debugging with minutes of focused investigation. It is a professional skill with an outsized return on the small time investment to learn it.",
            sections: [
                {
