const ModuleExpert = {
    description: "The frontier of C mastery. Variadic functions let you build APIs that accept any number of arguments — understanding them explains how printf actually works. Signal handling lets your program respond to OS events like Ctrl+C gracefully. Type-generic programming with _Generic achieves compile-time dispatch without C++ templates. C23's auto and typeof bring modern type inference. setjmp/longjmp provide non-local jumps for error handling without exceptions. Threads with <threads.h> unlock true parallelism. restrict and inline are the precision performance tools that expert C developers use to squeeze the last percentage points of speed. After this module, you will have covered the entire language, including its newest features.",

    lessons: [
        {
            id: "variadic-functions",
            title: "Variadic Functions",
            explanation: "Every call to <code>printf</code> is a call to a variadic function — one that accepts a variable number of arguments of varying types. C's type system normally requires functions to have a fixed, known signature, so variadic functions are a deliberate and carefully bounded hole in that system. After the fixed parameters, they accept any number of additional arguments of any types — and it is entirely the caller's responsibility to pass the right types and the right count. <code>&lt;stdarg.h&gt;</code> provides the machinery: a <code>va_list</code> iterator that walks through the extra arguments one at a time. You need to write variadic functions when building wrappers around <code>printf</code>, type-dispatching loggers, functions that accept a variable number of integers, or any API that naturally takes an open-ended list. The critical danger: C has no way to verify at compile time that the caller passed the right types and count. Getting the type wrong in <code>va_arg</code> is silent undefined behavior — which is exactly why printf uses a format string to encode expected types explicitly, letting the compiler check them for you.",
            sections: [
                {
...
int main(void) {
    ...
    return 0;
        ...
        },
        {
            ...
            explanation: "A signal is the operating system's mechanism for delivering an asynchronous event notification to a running process — it can arrive between any two instructions, with no warning and no predictable timing. Pressing Ctrl+C sends SIGINT. The <code>kill</code> command sends SIGTERM. Integer division by zero raises SIGFPE. Dereferencing a NULL pointer raises SIGSEGV. By default, most signals terminate the process immediately and unconditionally. Signal handlers let you intercept signals before that happens: to flush unsaved data, delete temporary files, log diagnostics, or perform graceful shutdown. The constraints are severe and non-negotiable. A signal handler executes asynchronously — it can interrupt your program mid-malloc, mid-printf, mid-anything. Most library functions are not async-signal-safe and cannot be called from inside a handler without invoking undefined behavior. The only universally correct pattern is: set a flag in the handler, return immediately, and let the main loop check the flag and react. Signal handling done right is genuinely tricky; done wrong it silently corrupts program state.",
            sections: [
                {
...
int main(void) {
    ...
    return 0;
        ...
        },
        {
            ...
            explanation: "C has always had a strong type system but historically no way to write code that adapts its behavior to types at compile time — short of separate function names (<code>abs</code>, <code>fabs</code>, <code>cabsf</code>) or macros with type-safety problems. C11 introduced <code>_Generic</code> to solve this. It is a compile-time selection construct: given an expression, select a different result based on the type of that expression. The unselected branches are discarded entirely — only the matching branch exists in the compiled output. There is zero runtime overhead; this is purely a compile-time dispatch. <code>_Generic</code> is the mechanism that makes <code>&lt;tgmath.h&gt;</code> work (type-generic math functions that automatically select <code>sinf</code>, <code>sin</code>, or <code>sinl</code> based on argument type), and it enables you to write single-name macros that dispatch to correctly typed implementations with full compiler type checking.",
            sections: [
                {
...
int main(void) {
    ...
    return 0;
        ...
        },
        {
            ...
            explanation: "A thread is an independent sequence of execution within a process that shares the same memory space as all other threads. Multithreading lets a program do multiple things at the same time: handle ten network connections concurrently instead of one at a time, compute results in the background while the main thread keeps the UI responsive, spread a CPU-bound workload across all cores of a modern multi-core processor. C11 standardized a portable threading API in <code>&lt;threads.h&gt;</code> — before C11, every platform had its own incompatible API (POSIX pthreads on Linux/macOS, Win32 threads on Windows). The fundamental hazard of multithreading is shared memory: two threads reading and writing the same variable simultaneously creates a data race — undefined behavior where the result depends on the arbitrary interleaving of operations from different CPU cores. Mutexes are locks that prevent data races by ensuring only one thread can execute a protected section at a time. Condition variables let threads sleep while waiting for a condition rather than burning CPU in a busy loop. These two primitives, understood well, are the foundation of all correct multithreaded C code.",
            sections: [
                {
